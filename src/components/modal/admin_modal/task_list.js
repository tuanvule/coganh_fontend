...
export default function Task_list({tasks, set_is_reset_task, set_task_chunk_index, is_reset_task}) {
  ...
  function handle_delete(id, author_id) {
    ...
    if (is_delete) {
      ...
      fetch(`http://127.0.0.1:8080/delete_task/${id}`,{
        headers: {
  ...
  function handle_accept(id, author_id, task_title) {
    fetch(`http://127.0.0.1:8080/accept_task/${id}`)
      .then(res => res.json())
      .then(data => {
        if(author_id) {
  ...
  useEffect(() => {
    fetch(`http://127.0.0.1:8080/get_unpublic_tasks?page=${UPT_chunk_index}&size=9`)
    .then(res => res.json())
    .then(data => {
        ...
        <tbody>
          {un_public_tasks && un_public_tasks.map((task, i) => 
            <tr key={i}>
              ...
              <td onClick={() => handle_message(task.author_id)} className="border border-slate-600 px-1 py-1 text-slate-200 text-xl text-center cursor-pointer hover:brightness-90"><i class="fa-solid fa-comment"></i></td>
