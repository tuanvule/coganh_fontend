...
export default function User_post({ username, posts, is_owner, set_is_require_owner }) {
  ...
  useEffect(() => {
    if (is_reset_post !== check) {
      fetch(`https://coganh-cloud-tixakavkna-as.a.run.app/get_post_by_username/${username}`)
        .then(res => res.json())
  ...
  useEffect(() => {
    fetch(`https://coganh-cloud-tixakavkna-as.a.run.app/get_unpublic_user_posts?username=${username}&page=${UPP_chunk_index}&size=9`)
      .then(res => res.json())
