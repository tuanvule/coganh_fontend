...
export default function Comment_modal({ post_id = null, task_id = null, bg = "bg-[#0e335b]", px = "px-4", set_comment_count, }) {
  ...
  useEffect(() => {
    ...
    if (post_id) {
      fetch(`http://192.168.1.249:8080/get_post_comments/${post_id}`)
        .then(res => res.json())
        .then(data => {
      ...
      fetch(`http://192.168.1.249:8080/get_task_comments/${task_id}`)
      .then(res => res.json())
      .then(data => {
        let comment = handle_data(data, "comment_time")
        set_comments(comment)
  ...
  function handle_send_comment() {
    if (user.username) {
      if (input_ref.current.value.length === 0) {
        ...
        fetch('http://192.168.1.249:8080/handle_comment', {
          method: 'POST',
          headers: {
